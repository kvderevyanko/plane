...
The preferred pusher integration is APC 13×10E, with propeller/motor axis at
`Z=+50 mm`, propeller plane `X=430 mm`, static ground plane `Z=-258 mm`, and
the two main wheel contact stations at `X=123 mm`, `Y=±170 mm`.  Main wheels
are Ø90 mm and the nose wheel is Ø70 mm.  These are preliminary packaging
points, not a selected wheel, leg or steering mechanism.

The supplied structural screen records 13-in tip clearance as 142.9 mm static,
128 mm with 15-mm compression, 85 mm at 8-degree tail-low stance, and 60 mm
in the full rough case (rut plus tolerance).  The last case meets the current
60-mm minimum study goal.  The 14-in propeller gives only 47 mm in the same
full-rough geometry; it is therefore excluded from the rough-field baseline.
...
`X=123, Y=±170, Z=-55 mm`.  The intended load path is 3-mm birch double-shear
...
can be represented consistently in the aircraft datum.  Detailed fuselage CAD
remains blocked until battery X travel/CG closure, battery removal/hatch route,
actual hardware envelopes, ski dimensions, wheel-leg load proof and final
boom/motor mounts are agreed.
